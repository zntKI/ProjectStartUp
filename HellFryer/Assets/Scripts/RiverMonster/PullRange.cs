...
public class PullRange : MonoBehaviour
{
    ...
    void DrawCircle(Vector3 center, float radius, int segments, Color color, float duration)
    {
        ...
        Vector3 prevPoint = center + new Vector3(Mathf.Cos(Mathf.Deg2Rad * angle) * radius, 0f, Mathf.Sin(Mathf.Deg2Rad * angle) * radius);
        angle += angleStep;

        for (int i = 1; i <= segments; i++)
        {
            Vector3 nextPoint = center + new Vector3(Mathf.Cos(Mathf.Deg2Rad * angle) * radius, 0f, Mathf.Sin(Mathf.Deg2Rad * angle) * radius);
